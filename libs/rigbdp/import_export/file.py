# builtins
import importlib, time, os, shutil, sys, re, json, glob, platform, fnmatch
...
# ################################################################################
def print_directory_map(root_folder, indent="", ignore_list=['.git', 'DS', 'DS_Store', 'gitignore', 'project', '.pydevproject',
                                                            'code-workspace', '.idea', 'vs', 'vscode', 'USD_maya_scenes',
                                                            'icons', 'builders']):
    """
    Prints the directory structure of the given root folder.

    Args:
        root_folder (str): The path of the folder to print.
        indent (str): The current indentation level for printing.
        ignore_list (list): A list of patterns to ignore (default is None to show everything).
    ...
    # #########################################
    """
    recursive_print_directory_map(root_folder, indent, 4, 14, ignore_list)

def recursive_print_directory_map(root_folder, indent="", num_padding=4, back_num_padding=14, ignore_list=None):
    """
    Prints the directory structure of the given root folder recursively, starting with the root folder.
    ...
    def _print_subdirs(folder, indent):
        ...
        # Separate files and directories, ignoring those in the ignore_list
        for item in items:
            item_path = os.path.join(folder, item)
            
            # Ignore the entire directory tree if the item is a directory and in the ignore list
            if os.path.isdir(item_path):
                if any(ignore in item for ignore in ignore_list):
