...
from CW import reload_maya_scene
from importlib import reload
reload(reload_maya_scene)
...
'''
TODO: add extra stationary tw joint that sits at end and twists 100%.
This is important for inheriting weights from the start.  For example elbow weights on the wrist
need to slide to the new 'elbow' aka twist end in order to remain stationary.

This twist end joint also acts as an anchor joint when we add the arm bends.

TODO: This new stationary twist joint must always be the final end joint for the wrist.  It is
important that you have a joint there that does not move in ry and rz! This will isolate and avoid
the wrist bowing that is currently happening!

make sure to multiply the inherited weight!
'''
...
class Weights:
    ...
    def get_maya_points(self, geo, indices):
        if cmds.objectType(geo) == 'transform':
            geo = cmds.listRelatives(geo, shapes=True)[0]
    ...
    def select_points_between_startend(self, geo, start, end):
        ...
        indices, p_vectors = self.points_between_startend(indices,
                                                        point_vectors,
                                                        start,
                                                        end)
    ...
    def get_influence_data(self, joints):
        ...
        for skin in all_skinclusters:
            ...
            print('get influence data line 121 skinCluster ', skin)
            print('get influence data line 121 geom ', geom)
    ...
    def get_joint_allweights(self, skinCluster, mesh, joint, round_amt=False):
        # the list returned by this method contains a weight value for every vertex in the mesh.
        # the result is a list that is safe for 'wildcard-type' point index lookups.
        ...
        # cpnt_indices = om.MIntArray(cpnt_indices)
        # Convert list of integers it to type kMeshVertComponent 
        cpnt_list = om.MFnSingleIndexedComponent().create(om.MFn.kMeshVertComponent)
        om.MFnSingleIndexedComponent(cpnt_list).addElements(cpnt_indices)
        # if for some reason the list isn't created, or fails, you won't want to move forward.
    ...
    def point_weight_intersection(self, joints, skinCluster, geo, threshold=.09):
...
def main():
    ...
    up_arm_jnts = ['RightArm','RightArm_out_bind_tw01', 'RightArm_out_bind_tw02', 'RightArm_out_bind_tw03', 'RightArm_out_bind_tw04', 'RightArm_out_bind_tw05',  'RightArm_out_bind_tw06', ]
    lo_arm_jnts = ['RightForeArm', 'RightForeArm_out_bind_tw01', 'RightForeArm_out_bind_tw02', 'RightForeArm_out_bind_tw03', 'RightForeArm_out_bind_tw04', 'RightForeArm_out_bind_tw05', 'RightForeArm_out_bind_tw06',]
    ...
    # inherit
# Example usage
reload_maya_scene.load_maya_scene()
main()
