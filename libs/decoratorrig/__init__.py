...
import asciiart
importlib.reload(asciiart)

# maya only decorators
try:
    # third party
    ...
    # not in maya no need to import from maya
    pass
...
def art(func):
    ...
    def wrapper(*args, **kwargs):
        # Get the current selection
        asciiart.RANDOM_TEXT
        # Get a random index from the list
        random_index = random.randint(0, len(asciiart.RANDOM_TEXT) - 1)

        # Retrieve the element at the random index
        random_string = asciiart.RANDOM_TEXT[random_index]
        print(random_string)
        try:
            # Run the decorated function
            return func(*args, **kwargs)
            ...
            asciiart.RANDOM
            # Get a random index from the list
            random_index = random.randint(0, len(asciiart.RANDOM) - 1)

            # Retrieve the element at the random index
            random_string = asciiart.RANDOM[random_index]
    return wrapper
