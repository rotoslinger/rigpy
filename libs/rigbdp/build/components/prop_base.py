...
MODS = [tag_utils, control_base, misc]
for mod in MODS:
    reload(mod)
...
class simple_component():
    def __init__(self,
        ...
        # This will only be set if the associate_geos args have been given
        ...
        # post build positioning and sizing
    ...
    def __finalize_associate_geos(self):
        ...
        for idx, name in enumerate(self.ctrl_names):
            ...
            # self.default_positions.append([center_x, center_y, center_z])
            # self.default_sizes.append(math.sqrt((width * 0.5)**2 + (height * 0.5)**2 + (depth * 0.5)**2))
        
        # print('ctrl_buffers: ', self.ctrl_buffers)
        # print('buffer_size_attrs: ', self.buffer_size_attrs)
        # self.ctrl_buffers
    ...
    def __create_shape_size(self):
        ...
        #set the default value to 1 if

        for idx, buffer in enumerate(self.ctrl_buffers):
            buffer = buffer[0] # --- only do it to the first buffer
            cmds.addAttr(buffer,
                        ln = "ctrl_siz", 
                        at = "float", 
                        dv = self.default_sizes[idx],
                        min = 0,
                        k = True)
